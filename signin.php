			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) > 1 ) {
						...
						$_POST['last_name'] = ucwords($_POST['last_name']);
						$_POST['email'] = ucwords($_POST['email']);
						$_POST['address'] = ucwords($_POST['signupaddress']);
						...
						";
						if (@mail($_POST['email'],"eBuyBD Activation Code", $msg)) {
							
						$result = mysqli_query($con, "INSERT INTO `user`(`firstName`, `lastName`, `email`, `mobile`, `address`, `password`, `confirmCode`, `activation`) VALUES ('$_POST[first_name]','$_POST[last_name]','$_POST[email]','$_POST[mobile]','$_POST[address]','$_POST[password]','$confirmCode', 'No')");
		                
		                //$result1 = mysqli_query($con, $result) or die(mysqli_error($con));

                        // if ($result1){ // I would usually use mysql_insert_id as a validation from auto_increment tables.
                        //     echo "kya mc raa ho b";
                        // } else {
		                ...
		                if(isset($_POST['first_name'])&&isset($_POST['last_name'])&&isset($_POST['email'])&&isset($_POST['mobile'])&&isset($_POST['address'])&&isset($_POST['password'])&&isset($confirmCode)){
		                    
		                    echo $_POST['first_name'].$_POST['last_name'].$_POST['email'].$_POST['mobile'].$_POST['address'].$_POST['password'];
		                }else{
		                    echo "Error";
		                }
						//success message
	...
	<body class="home-welcome-text" style="background-image: url(image/homebackgrndimg2.png);">
		<div class="homepageheader" style="position: inherit;">
			<div class="signinButton loginButton">
				...
				<div class="uiloginbutton signinButton loginButton" style="">
					<a style="text-decoration: none; color: #fff;" href="shops.php">NEAR-BY GROCERY SHOPS</a>
				</div>
			</div>
